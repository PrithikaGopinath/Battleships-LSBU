...
public class GamePanel extends JPanel {
    ...
    private enum State {
        WAITING_TO_START, RUNNING_GAME, GAME_OVER
    }
    ...
    public void prompt(final AbstractPlayer player) {
        if (player == this.game.getPlayer1()) {
            this.messageLabel
                    .setText(">>> " + player.getName() + " to bomb " + player.getOpponent().getName() + "! >>>");
            this.player1BoardPanel.setEnabled(false);
    ...
    public void displayResult(final AbstractPlayer bomber, final Outcome result) {
        ...
        final BoardButton b = boardPanel.geBoardButton(result.getX(), result.getY());
        if (result.hit()) {
            ...
            if (s == null) {
                ...
                if (result.gameWon()) {
                    this.messageLabel
                            .setText("HIT and " + s.getName() + " destroyed and " + bomber.getName() + " wins!");
                    this.player1BoardPanel.setEnabled(false);
                    this.player2BoardPanel.setEnabled(false);
                    this.player1BoardPanel.setShowShips(true);
                    this.player2BoardPanel.setShowShips(true);
        ...
        repaint(); // button repaints with current square status
    }
